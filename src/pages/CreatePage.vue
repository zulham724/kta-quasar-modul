          ...
          <div class="row">
            <div class="col-8 text-weight-medium q-gutters-none q-pa-sm">
              ...
              <q-btn
                ...
                color="white"
                no-caps
                @click="$router.push('/create/cover')"
                label="Pilih Sampul"
              >
            ...
            <div class="col-4 q-pa-sm text-center" style="background-color: white">
              <div class="vertical-middle" @click="editCover">
                ...
                <div class="text-caption">
                  {{ module.selected_template ? module.selected_template.name : null }}
                </div>
              </div>
            </div>
          </div>
          ...
          <q-card class="q-my-md" rounded>
            ...
            <q-card-section style="padding-top: 0px; padding-bottom: 0px">
              <q-btn
                ...
                flat
                dense
                class="q-px-sm text-weight-light"
                style="color: white; font-size: 12px; background-color: #840000"
              >
                  ...
                  <li>
                    <q-item style="border-top: 1px solid grey">
                      ...
                      <q-item-section
                        ...
                        class="q-px-xs"
                        clickable
                        @click="confirmDelete(n, content.name)"
                      >
                        <span
...
export default {
  ...
  computed: {
    ...
    selectedTemplate: function () {
      if (this.Module.build.canvas_image) return this.Module.build.canvas_image;
      else if (this.Module.build.selected_template)
  ...
  created: function () {
    ...
    if (this.Module.build.canvas_data.items.length == 0) {
    ...
    this.module = {
      ...
      contents: [
        ...this.Module.build.contents.map((content) => {
          return {
            ...content,
          };
        }),
    ...
    if (this.module.contents.length == 0) this.module.contents.push(this.content);
  },
  watch: {
  ...
  methods: {
    ...
    submitModule() {
        console.log('data to submit', this.module);
        // return;
      this.$store
        .dispatch("Module/store", this.module)
        .then((res) => {
          this.$store.commit("Module/resetBuild");
          this.$q.notify(
            res.data.is_publish
              ? "Berhasil menerbitkan modul"
              : "Berhasil menyimpan modul"
          );
        ...
        if (success) {
          this.$q
            .dialog({
              title: "Konfirmasi",
              message: is_publish
                ? "Modul akan disimpan dan dipublikasikan ke semua orang"
                : "Modul akan disimpan sebagai draft, tidak bisa dilihat oleh pengguna lain",
              cancel: true,
              persistent: true,
            })
            .onOk(() => {
              ...
              this.sampulMakerItems = [
                ...
                {
                  text: this.Module.build.grade
                    ? this.Module.build.grade.description
                    : null,
                  color: "#000000",
                  size: 8,
                  x_append: 400,
              ...
              const img2 = this.$q.platform.is.mobile && !process.env.DEV
                ? image
                : this.Setting.url + "/reverseproxy?url=" + image;
