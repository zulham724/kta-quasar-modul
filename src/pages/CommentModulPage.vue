<template>
  <div> 
    <q-header>
      <q-toolbar style="background-color:#840000">
        <q-btn flat dense icon="arrow_back" @click="$router.back()" />
        <q-toolbar-title>
          <div class="text-body2 text-weight-light" style="font-size:15px">Komentar</div>
        </q-toolbar-title>
      </q-toolbar>
    </q-header>
    <q-page class="q-pa-md" style="width:100%">
      <q-item class="" style="border-bottom:2px solid #eaeae1; 
        padding-left:0px; 
        padding-right:0px;
        padding-top: 0px
        width:100%"
      >
        <q-item-section avatar>
          <q-avatar class=" text-center" rounded size="18vw">
            <img src="https://cdn.quasar.dev/img/avatar.png" >
          </q-avatar>
        </q-item-section>
        <q-item-section top>
          <div class="text-weight-bold" style="font-size:16px">Bapak Jodi</div>
          <q-item-label
            class="text-weight-regular text-subtitle2"
            style="font-size:12px"
          >
            Pendidikan pendidik uk
          </q-item-label>
        </q-item-section>
      </q-item>
      <div class="q-py-sm text-weight-bold">
        Komentar
      </div>
      <q-item class="" style="padding-left:0px;padding-top: 0px; padding-right:0px">
        <q-item-section avatar>
          <q-avatar class=" text-center" rounded size="18vw">
            <q-img src="https://cdn.quasar.dev/img/avatar.png"></q-img>
          </q-avatar>
        </q-item-section>
        <q-item-section>
          <div class="text-weight-bold" style="font-size:15px">Bapak Jodi</div>
          <q-input rounded dense outlined label="Tulis Komentar...">
            <template v-slot:after>
              <q-btn round dense flat icon="send" />
            </template>
          </q-input>
        </q-item-section>
      </q-item>
      <div v-for="n in 3" :key="n" >
        <comment-modul></comment-modul>
      </div>
    </q-page>
  </div>
</template>

<script>
export default {
    components: {
        CommentModul: () => import("components/Modul/CommentModul.vue")
    },
    data() {
        return{
            text:"dajdnsjdnasjdnasjkdnaskdias,dnsakjdandusjbdhassaklndaskdnsajkdbsudhjbsa,dhasnduiadsj"
        }
    }
}
</script>