    ...
    <q-pull-to-refresh @refresh="refresh">
        <q-header>
            <q-toolbar style="background-color:#840000">
                ...
                <q-toolbar-title>
                    <div class="text-body2 text-weight-light" style="font-size:16px">Semua Modul</div>
                </q-toolbar-title>
            </q-toolbar>
            <q-toolbar class="q-pb-md q-pt-sm q-px-md" style="background-color:#840000">
                <div class="row full-width justify-between">
                    ...
                    <div class="col-4">
                        <q-select dense outlined rounded bg-color="white" v-model="model" style="width:100%" :options="getEducationalLevelOptions" option-label="name" option-value="id" label="Pilih kelas" />
                    </div>
                </div>
            </q-toolbar>
        </q-header>
...
export default {
    ...
    computed: {
        ...mapState(["Setting", "ModulesAll", "EducationalLevel"]),
        getModules: function () {
            let search = this.search.trim().toLocaleLowerCase();
            if (!this.ModulesAll.data.data) return [];
            return this.ModulesAll.data.data.filter(e => {
                let condition1 = search != '' ? (e.name ? e.name.toLocaleLowerCase().includes(search) : false) : true
                let condition2 = this.model.id > -1 ? e.grade.educational_level_id == this.model.id : true;
                return condition1 && condition2;
            })
        },
        getEducationalLevelOptions: function () {
            let data = [{
                id: -1,
                name: 'Semua'
            }];

            if (this.EducationalLevel.educationallevels.length == 0) return data;
            //console.log(this.EducationalLevel.educationallevels)
            return [...data, ...this.EducationalLevel.educationallevels];
        },
    ...
    created: function () {
        ...
        if (!this.EducationalLevel.educationallevels.length) this.$store.dispatch("EducationalLevel/index").then(res => {
            //console.log(res.data)
        });
    },
    methods: {
        onLoad(idx, done) {
            if (this.ModulesAll.data.next_page_url == null) done();
            else {
                this.$store.dispatch('ModulesAll/next', {
                    next_page_url: this.ModulesAll.data.next_page_url
                }).then(res => {
                    done();
                }).catch(err => {
                    alert(err)
