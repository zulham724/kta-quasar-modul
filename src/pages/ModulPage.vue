  ...
  <div style="background-color: #840000">
    <q-header>
      <q-toolbar style="background-color: #840000">
        ...
        <q-btn
          v-if="module.user.id == Auth.auth.id"
          color="white"
          round
          flat
          icon="more_vert"
              ...
              >
                <q-item-section>
                  <div clickable @click="$router.push('/edit/' + moduleId)">
                    <span class="material-icons" style="padding-right: 6px"> edit </span>
                    Sunting
                  </div>
                </q-item-section>
              </q-item>
          ...
          <q-item style="padding: 0px">
            ...
            <q-item-section side>
              <div
                ...
                "
                clickable
                @click="$router.push('/modulecomments/' + moduleId)"
              >
                <span class="material-icons" style="font-size: 24px"> comment </span>
            ...
            <q-item-section side style="padding-left: 10px">
              <div
                v-ripple:red
                @click="module.liked_count ? dislike(module.id) : like(module.id)"
                class="text-center q-pa-sm"
                style="
                  height: 55px;
                  width: 55px;
              ...
              >
                <span
                  :class="`material-icons ${module.liked_count ? 'text-red' : ''}`"
                ...
                <q-item class="full-width q-pa-none">
                  <q-item-section avatar>
                    <q-avatar rounded size="100px">
                      <q-img
                        v-if="module.user.avatar"
                        :src="`${Setting.storageUrl}/${module.user.avatar}`"
                      />
                    </q-avatar>
                  </q-item-section>
                  <q-item-section>
    ...
    <q-page      style="min-height: 0px; width: auto;" v-if="show">
            ...
            <q-item-section class="">
              <q-item-label class="q-px-xl q-py-md">
                <q-select
                  ...
                  option-label="name"
                  option-value="id"
                  :options="module.module_contents"
                  label="Sub Bab"
                />
              </q-item-label>
              <q-item-label>
              ...
              <q-item-label>
                <div v-html="getModuleContent.value"></div>
              </q-item-label>
            </q-item-section>
          </q-item>
              <div class="" v-if="getModuleContent.audio">
              <q-btn
    ...
    <q-footer
      v-if="!show && loading"
      style="background-color: white; height: 50px text-center"
    >
      <div class="row">
        <div class="col-4"></div>
...
export default {
  ...
  computed: {
    ...
    getModuleContent: function () {
      if (this.model) {
        const module_content = this.module.module_contents.find(
    ...
    if (!this.moduleId) {
      ...
      this.$store
        ...
        .then((res) => {
          ...
          this.model =
            this.module.module_contents.length > 0
              ? this.module.module_contents[0]
              : null;
          this.loading = false;
        })
        .catch((err) => {
  ...
  methods: {
    ...
    open() {
      if (this.module.module_contents.length > 0) this.show = true;
        ...
        this.$q.notify("Modul ini tidak mempunyai konten, silahkan kontak pembuat modul");
    },
    close() {
      this.show = false;
    },
    playAudio() {
      ...
      this.audio_player.item = new Audio(`${this.Setting.storageUrl}/${this.getModuleContent.audio.src}`);
